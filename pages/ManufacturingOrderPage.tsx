        ...
        <SectionCard title="المعلومات الأساسية" icon={DocumentTextIcon}>
            ...
            <FormRow>
                <FormField label="المسؤول عن التصنيع">
                    <select value={order.responsibleEmployeeId || ''} onChange={e => handleChange('responsibleEmployeeId', e.target.value)} className="form-select">
...
const FormulaBuilder = ({ order, setOrder, products, errors, onOpenAiModal }: { order: ManufacturingOrder, setOrder: (order: ManufacturingOrder) => void, products: Product[], errors: ValidationErrors, onOpenAiModal: () => void }) => {
    ...
    const addLine = () => {
        const newLine: FormulaLine = { id: Date.now().toString(), materialId: '', materialName: '', materialSku: '', kind: 'AROMA_OIL', percentage: 0 };
...
const PackagingPlanner = ({ order, setOrder, products, inventory }: { order: ManufacturingOrder, setOrder: (o: ManufacturingOrder) => void, products: Product[], inventory: InventoryItem[] }) => {
    ...
    const handleLineChange = (index: number, field: keyof PackagingItem, value: any) => {
        ...
        if (field === 'productId') {
            const material = packagingProducts.find(p => p.id === value);
            newItems[index] = { ...newItems[index], productId: value, name: material?.name || '' };
    ...
    const addLine = () => setOrder({ ...order, packagingItems: [...order.packagingItems, { productId: '', name: '', qtyPerUnit: 1 }] });
