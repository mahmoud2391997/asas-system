...
// FIX: Corrected the import path for AuthContext. It is defined in types.ts to avoid circular dependencies.
...
const PurchaseInvoices: React.FC<PurchaseInvoicesProps> = ({ invoices, onSave, branches, products, sales, inventory, suppliers }) => {
    ...
    const filteredInvoices = useMemo(() => {
        return invoices.filter(p => {
            ...
            const supplierMatch = filters.supplier === 'all' || p.supplierId === filters.supplier;
    ...
    const handleAddItem = () => {
        const newItem: PurchaseInvoiceItem = { id: Date.now(), productId: '', productName: '', quantity: 1, unitPrice: 0, total: 0 };
    ...
    const handleGenerateAISuggestions = async (branchId: string, forecastDays: number, historyDays: number): Promise<SuggestedPurchaseOrderItem[]> => {
        // ... (existing code)
        return [];
    };

    const handleCreatePoFromSuggestion = (items: { productId: string; quantity: number; unitPrice: number; }[]) => {
       // ... (existing code)
    };
        ...
        <>
                                ...
                                {filteredInvoices.map(p => {
                                    ...
                                    <React.Fragment key={p.id}>
                                        <tr style={{backgroundColor: isEditing ? 'var(--highlight-selected)' : 'transparent'}}>
                                            ...
                                            <td>{isEditing ? (
                                                <select value={editedInvoice?.supplierId} onChange={e => handleEditedDataChange('supplierId', e.target.value)} className="form-select">
                                                    {suppliers.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
                                                </select>
            ...
            {isAiModalOpen && (
                <AIPurchaseOrderModal 
                    ...
                    branches={branches}
                    onGenerate={handleGenerateAISuggestions as any}
