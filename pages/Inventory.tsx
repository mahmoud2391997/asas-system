...
interface InventoryProps {
    ...
    onTransferInventory: (data: { sourceBranchId: string; destinationBranchId: string; productId: string; quantity: number; }) => void;
    onAdjustInventory: (data: { branchId: string; productId: string; newQuantity: number; reason: AdjustmentReason; notes?: string; }) => void;
}
...
const Inventory: React.FC<InventoryProps> = ({ inventory, products, branches, onUpdateItem, onTransferInventory, onAdjustInventory }) => {
    ...
    const handleTransfer = (data: { sourceBranchId: string; destinationBranchId: string; productId: string; quantity: number; }) => {
        onTransferInventory(data);
        setTransferModalOpen(false);
        addToast('Inventory transferred successfully!', 'success');
    };

    const handleAdjust = (data: { branchId: string; productId: string; newQuantity: number; reason: AdjustmentReason; notes?: string; }) => {
    ...
    const filteredInventory = useMemo(() => {
        return inventoryWithProductInfo.filter(item => {
            const branchMatch = filterBranch === 'all' || item.branchId === filterBranch;
