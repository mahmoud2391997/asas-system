...
const PurchaseQuotationModal: React.FC<PurchaseQuotationModalProps> = ({ quotation, onClose, onSave, suppliers, products, rfqs }) => {
    ...
    const handleAddItem = () => {
        // FIX: productId is a string.
        const newItem: PurchaseQuotationItem = { productId: '', description: '', quantity: 1, unitPrice: 0, total: 0 };
            ...
            <div className="modal-content glass-pane" style={{ maxWidth: '50rem' }} onClick={e => e.stopPropagation()}>
                ...
                <div className="modal-body">
                     <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '1rem', marginBottom: '1.5rem' }}>
                        ...
                        <div><label className="form-label">المورد</label><select value={editableQuotation.supplierId} onChange={e => handleFieldChange('supplierId', e.target.value)} className="form-select"><option>اختر...</option>{suppliers.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}</select></div>
                        ...
                        <div><label className="form-label">مرتبط بـ RFQ</label><select value={editableQuotation.rfqId} onChange={e => handleFieldChange('rfqId', e.target.value)} className="form-select"><option>اختر...</option>{rfqs.map(r => <option key={r.id} value={r.id}>RFQ #{r.id}</option>)}</select></div>
                    </div>
