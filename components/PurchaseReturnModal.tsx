...
const PurchaseReturnModal: React.FC<{
    ...
    const handleAddItem = () => {
        // FIX: productId is a string.
        const newItem: PurchaseReturnItem = { productId: '', quantity: 1, unitPrice: 0, total: 0, reason: '' };
                    ...
                    <div className="form-section">
                        <div className="form-section-body">
                            <div className="form-grid-cols-4">
                                {/* FIX: Removed Number() conversion. */}
                                <div><label className="form-label required">المورد</label><select value={editableReturn.supplierId} onChange={e => handleFieldChange('supplierId', e.target.value)} className="form-select"><option>اختر...</option>{suppliers.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}</select></div>
