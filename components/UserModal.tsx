...
interface UserModalProps {
    ...
    branches: Branch[];
    PERMISSION_GROUPS: { [group: string]: { key: Permission, label: string }[] };
}

const UserModal: React.FC<UserModalProps> = ({ user, onClose, onSave, branches, PERMISSION_GROUPS }) => {
    ...
    useEffect(() => {
        setEditableUser(isCreating ? {
            ...
            permissions: [], // Start with empty and let user/role define it
        } : user);
    }, [user, isCreating]);
                ...
                <div className="modal-body">
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem', marginBottom: '1.5rem' }}>
                        ...
                        {needsBranch && (
                            <div>
                                ...
                                <select value={editableUser.branchId || ''} onChange={e => handleFieldChange('branchId', e.target.value)} className="form-select">
                    ...
                    <div style={{ borderTop: '1px solid var(--surface-border)', paddingTop: '1rem' }}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '1.5rem' }}>
                            {/* FIX: Explicitly type the parameters of the map function callback to prevent TypeScript from inferring them as `unknown`. */}
                            {Object.entries(PERMISSION_GROUPS).map(([groupName, permissions]: [string, { key: Permission, label: string }[]]) => (
