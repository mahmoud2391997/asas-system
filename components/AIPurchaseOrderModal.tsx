...
interface AIPurchaseOrderModalProps {
    ...
    onClose: () => void;
    branches: Branch[];
    // FIX: Changed branchId to string to match type definitions.
    onGenerate: (branchId: string, forecastDays: number, historyDays: number) => Promise<SuggestedPurchaseOrderItem[]>;
    // FIX: Changed productId to string to match type definitions.
    onCreatePO: (items: { productId: string; quantity: number; unitPrice: number; }[]) => void;
}
...
const AIPurchaseOrderModal: React.FC<AIPurchaseOrderModalProps> = ({ isOpen, onClose, branches, onGenerate, onCreatePO }) => {
    ...
    const handleGenerateClick = async () => {
        ...
        setStep('loading');
        try {
            // FIX: No need to convert branchId to Number.
            const result = await onGenerate(branchId, forecastDays, historyDays);
    ...
    // FIX: Changed productId to string.
