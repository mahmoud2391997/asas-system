...
const PurchaseOrderModal: React.FC<PurchaseOrderModalProps> = ({ order, onClose, onSave, suppliers, products, purchaseQuotations }) => {
    ...
    const handleAddItem = () => {
        // FIX: productId is a string.
        const newItem: PurchaseOrderItem = { productId: '', quantity: 1, unitPrice: 0, total: 0 };
            ...
            <div className="modal-content glass-pane" style={{ maxWidth: '50rem' }} onClick={e => e.stopPropagation()}>
                ...
                <div className="modal-body">
                     <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '1rem', marginBottom: '1.5rem' }}>
                        {/* FIX: Removed Number() conversion for string IDs. */}
                        <div><label className="form-label">المورد</label><select value={editableOrder.supplierId} onChange={e => handleFieldChange('supplierId', e.target.value)} className="form-select"><option>اختر...</option>{suppliers.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}</select></div>
                        ...
                        <div><label className="form-label">مرتبط بعرض سعر</label><select value={editableOrder.quotationId || ''} onChange={e => handleFieldChange('quotationId', e.target.value)} className="form-select"><option value="">لا يوجد</option>{purchaseQuotations.map(q => <option key={q.id} value={q.id}>Quotation #{q.id}</option>)}</select></div>
                    </div>
