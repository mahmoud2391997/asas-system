...
const PurchaseDetailModal: React.FC<PurchaseDetailModalProps> = ({ purchase, onClose, onSave, branches, products, suppliers, scannedTotal }) => {
    ...
    const [editablePurchase, setEditablePurchase] = useState<Partial<PurchaseInvoice>>(
        isCreating 
        ? {
            // FIX: Use string for ID-based fields
            id: '',
            brand: 'Generic',
    ...
    const handleHeaderChange = (field: keyof PurchaseInvoice, value: any) => {
        if (field === 'currency') {
            ...
            setEditablePurchase(prev => ({ ...prev, brand: value as 'Arabiva' | 'Generic', branchId: brandBranches[0]?.id || '' }));
        }
         else {
    ...
    const handleItemChange = (index: number, field: keyof PurchaseInvoiceItem, value: string | number) => {
        ...
        if (field === 'productId') {
            // FIX: Compare string to string, don't use Number()
    ...
    const handleAddItem = () => {
        const newItem: PurchaseInvoiceItem = {
            id: Date.now(),
            productName: '',
            // FIX: productId should be a string
            productId: '',
            quantity: 1,
            unitPrice: 0,
            total: 0,
                ...
                <div className="modal-body">
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '1.5rem', marginBottom: '1.5rem' }}>
                        <div className="glass-pane" style={{ padding: '1rem', display: 'flex', flexDirection: 'column', gap: '0.75rem'}}>
                            ...
                            {isEditing ? (
                                <select value={editablePurchase.supplierId} onChange={e => handleHeaderChange('supplierId', e.target.value)} className="form-select">
                             ...
                             {isEditing ? (
                                <>
                                    ...
                                    <div style={{display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem'}}>
                                        ...
                                        <select value={editablePurchase.branchId} onChange={e => handleHeaderChange('branchId', e.target.value)} className="form-select">
                            ...
                            <tbody>
                                {(editablePurchase.items || []).map((item, index) => (
                                    <tr key={item.id}>
                                        {isEditing ? (
                                            <>
                                                <td style={{padding: '0.5rem'}}><select value={item.productId} onChange={e => handleItemChange(index, 'productId', e.target.value)} className="form-select"><option value="">اختر منتج</option>{products.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}</select></td>
