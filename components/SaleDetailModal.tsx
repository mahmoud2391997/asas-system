...
const SaleDetailModal: React.FC<SaleDetailModalProps> = ({ sale, onClose, onSave, branches, products, inventory, customers }) => {
    ...
    useEffect(() => {
        ...
        setEditableSale(isCreating ? {
            brand: defaultBrand,
            branchId: brandBranches[0]?.id || '',
            customerName: 'زبون نقدي عام',
            // FIX: Use string for ID
    ...
    const handleFieldChange = (field: keyof Sale, value: string) => {
        if (field === 'brand') {
            ...
            setEditableSale(prev => ({ ...prev, brand: value as 'Arabiva' | 'Generic', branchId: brandBranches[0]?.id || '' }));
        } else {
                ...
                <div className="modal-body">
                    <div style={{display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '1rem', marginBottom: '1rem'}}>
                        ...
                        <div>
                            ...
                            <select value={editableSale.branchId || ''} onChange={e => handleFieldChange('branchId', e.target.value)} className="form-select">
